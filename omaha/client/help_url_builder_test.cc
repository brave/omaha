...
const CString kExpectedBraveUrlBegin = _T("https://support.brave.com/");
const CString kExpectedBraveUrlAfterOs = _T("");
...
TEST_F(HelpUrlBuilderTest, BuildGetHelpUrl_User) {
  ...
  EXPECT_STREQ(kExpectedBraveUrlBegin, url.Left(kExpectedBraveUrlBegin.GetLength()));
  EXPECT_NE(-1, url.Find(kExpectedBraveUrlAfterOs))
...
TEST_F(HelpUrlBuilderTest, BuildGetHelpUrl_InstallerErrorWithExtraCode) {
  ...
  EXPECT_STREQ(kExpectedBraveUrlBegin,
               url.Left(kExpectedBraveUrlBegin.GetLength()));
  EXPECT_NE(-1, url.Find(kExpectedBraveUrlAfterOs))
      << kExpectedUrlAfterOs.GetString() << std::endl
      << _T(" not found in ") << std::endl << url.GetString();
}
